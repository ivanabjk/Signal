...
function updateCamera(kai, levelWidth, cameraRegions = null) {
  ...
  if (camera.bounds) {
    ...
    if (camera.x > camera.bounds.maxX - canvas.width) camera.x = camera.bounds.maxX - canvas.width;
    if (camera.y < camera.bounds.minY) camera.y = camera.bounds.minY;
    if (camera.y > camera.bounds.maxY - canvas.height) camera.y = camera.bounds.maxY - canvas.height;
  } else {
